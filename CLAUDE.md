...
**Mail goes out over one connection that is already logged in.** [EmailConfiguration](backend/src/main/java/pl/myproject/kanbanproject2/config/EmailConfiguration.java) builds a `PersistentSmtpMailSender`, a `JavaMailSenderImpl` that opens its transport at `ApplicationReadyEvent` and keeps it, instead of running a TCP + STARTTLS + AUTH handshake per message — that handshake used to sit on the critical path of every signup, resend and password reset. A scheduled ping (`app.mail.keep-alive-interval-ms`, four minutes) stops the server retiring it as idle and reopens it when it has; `SMTPTransport.isConnected()` is a `NOOP` on the wire, so the check before each send is a real one. Sends are serialised on the one transport, and a send that dies mid-write is retried once on a fresh connection while a message the server *rejects* is not — the two are told apart by whether the connection survived, never by the exception's type. That distinction is the whole of it: Gmail retires a held connection with `421 Connection expired, try reconnecting`, which Angus reports as `SMTPSendFailedException`, a `SendFailedException` — reading the class instead of the link turned the one failure this class exists to absorb into a failed signup. Connections are also replaced once they reach `app.mail.max-connection-age-ms` (ten minutes), so that expiry is normally met on the scheduler thread rather than in somebody's request. Declaring this bean is also what makes Boot's mail auto-configuration back off, which is why the class binds `MailProperties` explicitly: otherwise `spring.mail.*` in `application.properties` is read by nothing, which is how the host and port came to be hardcoded and `mail.debug` came to be pinned on.
